import cv2
import numpy as np
import matplotlib as plt
import math

def angle_cos(p0, p1, p2):
    d1, d2 = (p0-p1).astype('float'), (p2-p1).astype('float')
    return abs( np.dot(d1, d2) / np.sqrt( np.dot(d1, d1)*np.dot(d2, d2) ) )

def find_squares(img):
    img = cv2.GaussianBlur(img, (5,5), 0)
    img = cv2.morphologyEx(img, cv2.MORPH_OPEN, (5,5))
    squares = []
    bin = cv2.Canny(img, 100, 250, apertureSize=5)
    bin = cv2.dilate(bin, None)
    retval, bin = cv2.threshold(img, 100, 255, cv2.THRESH_BINARY)
    contours, hierarchy = cv2.findContours(bin, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)

    i1 = 0
    for cnt in contours:
    	children = 0
        cnt_len = cv2.arcLength(cnt, True)
        cnt = cv2.approxPolyDP(cnt, 0.02*cnt_len, True)
        if len(hierarchy[0]) > 0:
            i2 = hierarchy[0][i1][2]
            while i2 != -1:
                i2 = hierarchy[0][i2][0]
                children += 1

        i1 += 1
        if children >= 4 and len(cnt) == 4 and cv2.contourArea(cnt) > 100 and cv2.contourArea(cnt) < 250000 and cv2.isContourConvex(cnt):
            cnt = cnt.reshape(-1, 2)
            max_cos = np.max([angle_cos( cnt[i], cnt[(i+1) % 4], cnt[(i+2) % 4] ) for i in xrange(4)])
            if max_cos < 0.1:
                squares.append(cnt)

    return squares


cap = cv2.VideoCapture(0)

while(True):
	ret, img = cap.read()
	img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
	squares = find_squares(img)
	img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
	cv2.drawContours( img, squares, -1, (0, 255, 0), 2 )
	cv2.imshow('squares', img)

	if cv2.waitKey(1) & 0xFF == ord('q'):
		break

cap.release
cv2.destroyAllWindows()